...
class 移動クラス:
    ...
    def 実行(self):
        ...
        elif self._情報.プレイ時間 % self._仕様.難易度間隔 == 0:
        #　├┐（レベルの変わり目の場合）
            #↓
            #○ボスシーンを『ボス警告』に進行する
            #○ボス警告カウントを開始する
            #○プレイ時間を1つ進める ※ボスシーンを繰り返さない為
            self._情報.ボスシーン = DS.仕様.シーンID.ボス警告
        ...
        #　└┐（その他）
            ...
            #○難易度をセットする
            self._情報.プレイ時間 += 1
            self._情報.難易度 = self._情報.プレイ時間 // self._仕様.難易度間隔 + 1
        #┴　┴
    ...
    def Fnボス処理(self):
        ...
        #│
        #◇┐ボスシーンを進行する
        if self._情報.ボスシーン == DS.仕様.シーンID.ボス警告:
        #　├┐（ボス警告の場合）
            #↓
            #○警告時間を減らす
            ...
            #○タイマーをリセットする
            #●BGMを切替える
            self._情報.ボスシーン = DS.仕様.シーンID.ボス登場
            self.情報.開始時間 = None
            共通部品.BGM_DFP.指定曲ボス()
            #│
            #●特殊効果を解除(永続は残す)
            ...
            DS.obj.自機共通.情報.シールド = self._仕様.画面幅
        ...
        elif self._情報.ボスシーン == DS.仕様.シーンID.ボス対決:
        #　├┐（ボス対決の場合）
            #↓
            #〇ボスを確認する
