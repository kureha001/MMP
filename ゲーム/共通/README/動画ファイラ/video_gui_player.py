...
# ====== VLC 読み込み（libvlc.dll と plugins はこのファイルと同じフォルダ想定） ======
...
class VlcTkPlayer:
    def __init__(
        ...
        *,
        ...
        self._zoom_method = None  # "state" / "attr" / None(geometry)
        self._fs_prev_geom = None
        self._fs_method = None    # "tk" / "vlc" / "override"
        self._is_fs = False
        ...
        # ▼ OSD系を抑止
        vlc_args.append("--no-video-title-show")   # 再生開始時のファイル名/パスOSDを非表示
        vlc_args.append("--no-snapshot-preview")   # スナップショット時のOSD＆左上プレビューを非表示
        vlc_args.append("--no-osd")                # OSD全般を無効化（音量OSDや保存パス表示も消える）

        self.instance = vlc.Instance(vlc_args)
        self.player = self.instance.media_player_new()

        # ▼ ロゴ/マーキーのオーバーレイを完全無効化
        try:
        ...
        # 初期ミュート/音量（起動時の“鳴り”回避）
        self._desired_volume = int(self._default_volume)
        try:
            if self.mute_var.get():
                self.player.audio_set_volume(0)
                self.player.audio_set_mute(True)
                self.player.audio_set_volume(self._desired_volume)  # 解除時用に仕込む
            else:
    ...
    def _prepare_preview_frame(self):
        ...
        def _pause_when_ready():
            ...
            if st not in (vlc.State.Playing, vlc.State.Buffering):
                self.root.after(50, _pause_when_ready); return
            try:
        ...
        self.root.after(50, _pause_when_ready)

    # ---- スナップショット（最後のフレーム用キャッシュ）----
...
def open_video_menu(
    ...
    *,
    ...
    """
    呼び出し元から渡された動画リストでメニューを表示し、選択して再生します。
    """
    if not isinstance(videos, dict):
        raise TypeError("videos は dict で指定してください")

    def start_player(path: str, title: str):
        # URL の直リンク以外はブラウザで開いてメニューに戻す
        if URL_RE.match(path) and not any(path.lower().endswith(ext) for ext in STREAM_EXTS):
    ...
    def show_menu():
        ...
        tree.bind("<Double-1>", lambda e: start_selected())

        btn_row = ttk.Frame(menu_root)
        btn_row.pack(pady=6)
        ttk.Button(btn_row, text="再生", command=start_selected).pack(side="left", padx=6)
