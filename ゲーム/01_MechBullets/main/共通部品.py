...
class 入出力():
    ...
    MMP_BTN = (                         # Chごとの閾値(下限,上限) 
            (20        , 100*分解差),   # ハットスイッチ
            (450*分解差, 500*分解差),   # ボタン１
            (260*分解差, 300*分解差),   # ボタン２
            (50        , 100*分解差))   # ボタン３（発射ボタン）
    ...
    def MMP_移動(引数_番号):
		...
		#│
        中央値  = DS.仕様.ハード.MMP中央値
        閾値    = (DS.情報.操作反応率/100) * 中央値
        下限    = 中央値 - 閾値
        上限    = 中央値 + 閾値
		#│
        #○縦移動を測定す
        入力値  = MMP.接続.ANALOG.ROUNDD(引数_番号,0,丸め,DS.仕様.ハード.MMP分解能)
        測定[0] = (1) if 入力値 < 下限 else (0)
        測定[1] = (1) if 入力値 > 上限 else (0)
		#│
        #○横移動を測定する
        入力値  = MMP.接続.ANALOG.ROUNDD(引数_番号,1,丸め,DS.仕様.ハード.MMP分解能)
    ...
    def MMP_発射(引数_ポートNo):
        ...
        #○ポートのアナログ値を用意する
        ポートNo = 引数_ポートNo
        入力値 = MMP.接続.ANALOG.ROUNDD(ポートNo,2,DS.仕様.ハード.HC4067の丸め,DS.仕様.ハード.MMP分解能)
		#│
		#◎└┐ON/OFF状態を求める
        for 各Ch in range(4):
            #│
