...
//━━━━━━━━━━━━━━━━━
  ...
  // フレーム書式    ：{認証コード}/{コマンドパス}!
  // データ受信単位  ：フレーム単位
  // フレーム化処理  ：しない ※WebServer[conn->uri()]で実施済
  // 受信バッファ    ：しない
  // 受信継続判定    ：しない
  // フレーム終端判定：する   ※今後末尾文字で判定の予定
  //─────────────────
  ...
  //○┐３．事前準備
    ...
    ctx.cmdPath    = GetToken2(argSS.rx); // ★CONTEXT更新：コマンドパス
    argSS.rx = "";
    //┴
  //│
  //○┐４．認証を実施
    ...
    if (authCD == "_START_!") {
      //├┐（例外的に認証コードの中身が「接続開始コマンド」の文字列の場合）
        ...
        SEND_CONN(argSS, String("$") + newAuthCD + "$"); // 前後に識別文字を付加
        return;
      //┴
    } /*【4-1.認証開始要求を処理】*/
    //│
    //○4-2.ユーザ認証を実施
      ctx.authID = AUTH_GET_ID(AUTH_TBL, authCD); // ★CONTEXT更新：認証ID
      if (ctx.authID < 0) {
      // ＼（認証に失敗した場合）
        //●エラーコードをレスポンス
        //▼ルーティング処理を中断
...
//========================================================
  ...
  void handle() {
    ...
    // 3) ルーティング処理
    ADP_SETUP_CTX(ROUTE_ID, 0)  ; // コンテクストをセットアップ
