...
def RunDigital():
    ...
    print("　・出力[17]")
    for _ in range(3):
        _, _, j1, _ = _api_get(f"{命令}OUTPUT:17:1")
        print(f"　　・[HIGH] {tf(j1.get("result"))}"); time.sleep(0.5)
        _, _, j0, _ = _api_get(f"{命令}OUTPUT:17:0")
...
def RunPwm(argMode=True):
    ...
    print("　・初期位置")
    if argMode:
        _api_get(f"{命令PWM}:{CH_180}:{PWM_MID_180}")
        _api_get(f"{命令PWM}:{CH_360}:{PWM_MID_360}")
    else:
    ...
    for pwmVal in range(PWM_180[0], PWM_180[1], STEP):
        if argMode: _api_get(f"{命令PWM}:{CH_180}:{pwmVal}")
    ...
    for pwmVal in range(PWM_MID_360, PWM_360[0][1], STEP):
        if argMode: _api_get(f"{命令PWM}:{CH_360}:{pwmVal}")
...
def RunPwmSweep(命令, ch, start, end, step, delay):
    ...
    for pwmVal in rng:
        _api_get(f"{命令}OUTPUT:{ch}:{pwmVal}")
        time.sleep(delay)
...
def RunPwm_Angle():
    ...
    print("　・設定(登録)")
    _api_get(f"{命令}SETUP:{CH_180}:-1:{ANGLE_MAX}:{PWM_180[0]}:{PWM_180[1]}")

    print("　・角度：0度")
    _api_get(f"{命令}OUTPUT:{CH_180}:0")
    time.sleep(PAUSE_S)
    ...
    print("　・角度：中心")
    _api_get(f"{命令}OUTPUT:{CH_180}:{ANGLE_MAX // 2}")
    time.sleep(PAUSE_S)

    print("　・設定削除")
    _api_get(f"{命令}RESET:{CH_180}:-1")
...
def RunPwm_Rotate():
    ...
    print("　・初期化")
    _api_get(f"{命令}SETUP:{CH_360}:-1:{PWM_360[0][0]}:{PWM_360[0][1]}:{PWM_360[1][0]}:{PWM_360[1][1]}")

    print("　・停止")
    _api_get(f"{命令}OUTPUT:{CH_360}:{0}")
    time.sleep(PAUSE_S)
