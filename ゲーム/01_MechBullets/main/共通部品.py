...
class 入出力():
    ...
    def MMP_移動(引数_番号):
		...
		#│
        #○縦移動を測定す
        入力値  = MMP.接続.ANALOG.ROUNDD(引数_番号,0,丸め,DS.情報.分解能)
        測定[0] = (1) if 入力値 < 下限 else (0)
        測定[1] = (1) if 入力値 > 上限 else (0)
		#│
        #○横移動を測定する
        入力値  = MMP.接続.ANALOG.ROUNDD(引数_番号,1,丸め,DS.情報.分解能)
    ...
    def MMP_発射(引数_ポートNo):
        ...
        #○ポートのアナログ値を用意する
        ポートNo = 引数_ポートNo
        入力値 = MMP.接続.ANALOG.ROUNDD(ポートNo,2,DS.仕様.ハード.HC4067の丸め,DS.情報.分解能)
		#│
		#◎└┐ON/OFF状態を求める
        for 各Ch in range(4):
            #│
