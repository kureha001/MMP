...
//【内部利用】
    ...
    bool MmpClient::Open(
        ...
        // ★追加：内部Openは外部アタッチではない
        _attachedExternal = false; // ★追加
...
//【外部公開】
    ...
    // ★追加：外部でopen済みのシリアルにアタッチ（beginしない）
    bool MmpClient::AttachOpenSerial(HardwareSerial* ser) {  // ★追加
        Close();                                             // 既存を一旦解放（TCPや内部オープンを止める）
        if (!ser) { _lastError = "AttachOpenSerial: null"; return false; }
        _serial = ser;
        _handle = ser;
        _isOpen = true;
        _attachedExternal = true;                            // ★外部アタッチフラグ
        ClearInput();
        _connected_port = "UART(attached)";
        _connected_baud = 0;                                // 実効ボーレートは外部設定
        _lastError = "";
        return true;
    }                                                        // ★ここまで追加
    ...
    void MmpClient::Close(){
        ...
        if (_isOpen) {
            ...
            if (_serial && _handle == _serial) {
                _serial->flush();
                // ★外部アタッチ時は end() しない（Bridgeの常時オープンを壊さない）
                if (!_attachedExternal) {                   // ★追加
                    _serial->end();                         // ★追加
                }                                           // ★追加
            }
            ...
            _handle = nullptr;
            _isOpen = false;
            _attachedExternal = false;                      // ★追加：リセット
        }
    }
...
//============================================================
    ...
    bool MmpClient::OpenTcp(
        ...
        _attachedExternal = false; // ★TCPは外部アタッチではない
