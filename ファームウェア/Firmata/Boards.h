...
#elif defined(TARGET_RP2040) || defined(TARGET_RASPBERRY_PI_PICO_2) || defined(RASPBERRY_PI_PICO_2)
...
//-----------------------------------------------------------------
// Arduino UNO R4
#elif defined(RENESAS_RA4M1_firmata) || defined(ARDUINO_UNOWIFIR4)
#define TOTAL_ANALOG_PINS       6
#define TOTAL_PINS              20
#define VERSION_BLINK_PIN       13 
#define IS_PIN_DIGITAL(p)       ((p) >= 0 && (p) <= 13) 
#define IS_PIN_ANALOG(p)        ((p) >= 14 && (p) <= 19) 
#define IS_PIN_PWM(p)           (((p) == 3) || ((p) >= 5 && (p) <= 9) || ((p) >= 10 && (p) <= 13)) 
#define IS_PIN_SERVO(p)         (((p) == 3) || ((p) >= 5 && (p) <= 9) || ((p) >= 10 && (p) <= 13))
#define IS_PIN_I2C(p)           ((p) == 18 || (p) == 19)
#define IS_PIN_SPI(p)           ((p) == SS || (p) == MOSI || (p) == MISO || (p) == SCK) 
#define PIN_TO_DIGITAL(p)       (p)  
#define PIN_TO_ANALOG(p)        ((p)-14) 
