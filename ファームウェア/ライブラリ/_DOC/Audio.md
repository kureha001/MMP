...
**書式**：
- `ushort   Info.CONNECT(int     devId1to4, int     timeoutMs = 0)`
- `uint16_t Info.CONNECT(int32_t devId1to4, int32_t timeoutMs = 0)`
- `         Info.CONNECT(        devId1to4,         timeoutMs = 0)`

| 引数名    | 値  | 解説 |
|-----------|-----|------|
|`devId1to2`|1〜2 | DFPlayerのデバイスID|
|`timeoutMs`|0～  |応答待ちの時間(単位：ミリ秒)|
