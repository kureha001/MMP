...
class 本体:
    ...
    def 実行(self):
        #┬
        プレイ時間 = (200) if DS.情報.チート.時短 else (DS.仕様.プレイ時間)
    ...
    def Fnボス処理(self):
        ...
        #　├┐（ボス警告の場合）
            ...
            共通.音声.自動再生(DS.仕様.シーンID.プレイ_ボス画面, DS.情報.難易度 + 200)
            #│
            #●特殊効果を解除(永続は残す)
            #○シールドを満タンにする
            #○弾を満タンにする
        ...
        #　├┐（ボス対決の場合）
            ...
            #●BGMを切替える
            DS.情報.ボスシーン = None
            共通.音声.自動再生(DS.仕様.シーンID.プレイ_ザコ画面, DS.情報.難易度 + 100)
            共通部品.効果音.爆発(2)
            #│
            #○シールドを満タンにする
            #○弾を満タンにする
