#=====================================================================
# ＭＭＰライブラリ Rottenmeier用 MicroPython版
#---------------------------------------------------------------------
...
class mmp:
    ...
    def __init__(
        self,
        arg読込調整     = 64    , # 読込みバッファ(32～64[推奨64])
        arg通信速度     = 115200, # 通信速度(単位：bps)
        arg時間切れ     = 100   , # タイムアウト(単位：ミリ秒)
        arg割当ピンTX   = 0     , # TXピン
        arg割当ピンRX   = 1     , # RXピン
        argシリアル番号 = 0     , # 使用するUARTのID(0/1/2...)
        ):
        #┬
        #○1回でまとめて読む最大バイト数
    ...
    def 通信接続(self):
        ...
        try:
            self.uart = UART(
                ...
                parity       = None            , # パリティ
                stop         = 1               , # ストップビット
                tx           = Pin(self.tx_pin), # TXピン
                rx           = Pin(self.rx_pin), # RXピン
                )
        except Exception as e:
            raise ConnectException(f"UARTに接続失敗: {e}")

        # バッファをクリア
        time.sleep_ms(5) # 単位：ミリ秒
        self.バッファ消去()
