...
#────────────────────────────────────
def 自動再生(
        引数_シーンID   = 0,  #① 一覧の検索レベル(0/1/2/3)
        引数_検索キー   = 0,  #② 一覧の検索キー(0/1/2/3)
    ...
    #│
    #○ファイル一覧を求める
    フォルダNo = 情報.フォルダNo一覧[シーンID      ]
    一覧       = 情報.ファイルNo一覧[フォルダNo - 1]
    #│
    #○再生パラメータセットを用意する
    if 検索キー not in 一覧 : データ = 一覧[-1]
    else                    : データ = 一覧[検索キー]
    #│
    #○個別の再生パラメータを求める
    ファイルNo = (検索キー) if データ[0] == 0 else (データ[0])    
    音量       = (15      ) if データ[1] == 0 else (データ[1])
    時間       = データ[2]
    #│
    #○求めたパラメータで情報を更新する
    情報.シーンID   = シーンID
